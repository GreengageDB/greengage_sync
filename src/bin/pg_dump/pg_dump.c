...
dumpFunc(Archive *fout, const FuncInfo *finfo)
{
	 ...
	 * is used.  In GPDB Paris and up, an unused probin is NULL (here "");
	 * previous versions would set it to "-".  There are no known cases in
	 * which prosrc is unused, so the tests below for "-" are probably
	 * useless.
	 */
	if (prosqlbody)
	{
