...
# walsender releases the slot -- so it must not be run synchronously here.
my ($checkpoint_in, $checkpoint_out) = ('', '');
my $checkpoint_timer = IPC::Run::timeout(180);
my $checkpoint = $node_primary3->background_psql('postgres',
	\$checkpoint_in, \$checkpoint_out, $checkpoint_timer);
