...
BEGIN
	...
	FOR r IN SELECT colname, oper, typ, value[ordinality], matches[ordinality] FROM brinopers_multi, unnest(op) WITH ORDINALITY AS oper order by colname, typ, oper LOOP
...
alter table brin_test_multi alter column b set statistics 1000;
INSERT INTO brin_test_multi SELECT x/100,x%100 FROM generate_series(1,200000) x(x);
