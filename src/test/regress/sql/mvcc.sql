...
BEGIN
    ...
    -- in MPP the loop and per-cycle width must also be large enough that cumulative
    -- page splits from a reused stale local snapshot on the segment outpace the
    -- occasional background-triggered snapshot rebuild
    FOR i IN 1..400 LOOP
        BEGIN
	    -- perform index scan over all the inserted keys to get them to be seen as dead
            IF EXISTS(SELECT * FROM clean_aborted_self WHERE key > 0 AND key < 401) THEN
	        RAISE data_corrupted USING MESSAGE = 'these rows should not exist';
            END IF;
            INSERT INTO clean_aborted_self SELECT g.i, 'rolling back in a sec' FROM generate_series(1, 400) g(i);
