...
--
CREATE TABLE large_tuple_test (a int, b text) WITH (fillfactor = 10) DISTRIBUTED REPLICATED;
...
-- should still fit on the (slightly-filled) first page; the tuple sizes below
-- are relative to block_size so the page choreography is independent of
-- --with-blocksize
INSERT INTO large_tuple_test (select 2, repeat('a', (current_setting('block_size')::int / 8)));
-- GPDB: the table is replicated, so pg_relation_size() sums one page per primary
-- segment; compare against block_size * nsegments instead of a hard-coded size
...
INSERT INTO large_tuple_test (select 4, repeat('a', (current_setting('block_size')::int - 66)));
-- each primary segment now holds exactly three pages
SELECT pg_relation_size('large_tuple_test'::regclass, 'main') /
       current_setting('block_size')::bigint /
       (SELECT count(*) FROM gp_segment_configuration WHERE role = 'p' AND content >= 0)
       AS pages_per_seg;
