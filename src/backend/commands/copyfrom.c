			  ...
			  List *options)
{
	 ...
	 * conversion pipeline (raw_buf -> input_buf), the parser requires the
	 * data to be in the database encoding, so the QD now converts before
	 * parsing and dispatch; the QEs receive server-encoded lines.  In
	 * COPY_EXECUTOR mode the QD->QE stream is read directly via
	 * CopyGetData(), bypassing this pipeline, so this setup is inert
	 * there.
	 */
