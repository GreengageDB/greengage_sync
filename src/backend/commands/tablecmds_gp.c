...
AtExecGPSplitPartition(Relation rel, AlterTableCmd *cmd)
{
	...
	/* detach current partition */
	{
		...
		if (partrelid == get_default_oid_from_partdesc(RelationGetPartitionDesc(rel, true)))
	...
	switch (cmd->subtype)
	{
		...
		case AT_PartAdd:			/* Add */
		{
			...
			do
			{
				...
				partdesc = RelationGetPartitionDesc(temprel, true);
					...
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
								errmsg("level %d is not partitioned and hence can't set subpartition template for the same",
									   level)));
				if (RelationGetPartitionDesc(firstrel, true)->nparts == 0)
					ereport(ERROR,
							(errcode(ERRCODE_INVALID_OBJECT_DEFINITION),
								errmsg("GPDB SET SUBPARTITION TEMPLATE syntax needs at least one sibling to exist")));

				/* if this is not leaf level partition then sub-partition must exist for next level */
				if (!RelationGetPartitionDesc(firstrel, true)->is_leaf[0])
				{
