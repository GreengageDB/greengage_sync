...
CreateFunction(ParseState *pstate, CreateFunctionStmt *stmt)
{
	 ...
	 * below scribbles on stmt->sql_body (transformStmt is destructive on raw
	 * trees: e.g. a SubLink's subselect is replaced in place by the
	 * transformed Query, which a QE's own parse analysis then rejects with
	 * "unexpected non-SELECT command in SubLink").  Snapshot the statement
	 * now and dispatch the pristine copy.
	 */
