...
XLogBeginRead(XLogReaderState *state, XLogRecPtr RecPtr)
{
	...
	/* Forget any aborted-record state from a previous read. */
			   ...
			   char **errormsg)
{
	...
	for (;;)
	{
		...
		if (state->decode_queue_tail)
		{
			 ...
			 * page header, though.
			 *
			 * GPDB: it may also be further away, if the record we were
			 * reading was aborted and overwritten
			 * (XLP_FIRST_IS_OVERWRITE_CONTRECORD); overwrittenRecPtr is set
			 * in that case, and cleared again when the OVERWRITE_CONTRECORD
			 * record is verified during replay.
			 */
			Assert(state->record->lsn == state->EndRecPtr ||
				   ...
				   !XLogRecPtrIsInvalid(state->overwrittenRecPtr));
...
#ifndef FRONTEND
		/*
		 * GPDB: a garbage xl_tot_len (torn WAL or recycled-segment data on a
		 * recovering mirror) can push required_space past MaxAllocSize;
		 * palloc_extended would then FATAL despite MCXT_ALLOC_NO_OOM (the
		 * flag only covers OOM, not the size check).  Treat it as an invalid
		 * record instead, like allocate_recordbuf does.
		 */
		if (!AllocSizeIsValid(required_space))
			return NULL;
#endif
...
restart:
	switch (state->readRecordState)
	{
		case XLREAD_NEXT_RECORD:
			...
			/*
			 * We're starting to decode a fresh record, so forget about any
			 * incomplete record we might have given up on earlier.  (GPDB:
			 * part of the backported handling for records aborted at end of
			 * WAL, cf. XLP_FIRST_IS_OVERWRITE_CONTRECORD.)
			 */
			state->abortedRecPtr = InvalidXLogRecPtr;
			state->missingContrecPtr = InvalidXLogRecPtr;
		...
		case XLREAD_CONTINUATION:
			{
				...
				while (state->recordRemainLen > 0)
				{
					...
					{
						/*
						 * If the WAL ends before the caller can supply the
						 * next page, this record never completes.  Remember
						 * where the broken record started and where its first
						 * missing contrecord page is, so that if this turns
						 * out to be the end of WAL, replay knows where the
						 * aborted record was and where to direct followup WAL
						 * to be written, marking the next piece with
						 * XLP_FIRST_IS_OVERWRITE_CONTRECORD.  These are
						 * cleared again when decoding of the next record
						 * begins.
						 */
						state->abortedRecPtr = state->DecodeRecPtr;
					 ...
					 * "overwrite contrecord" flag, that means the
					 * continuation record was overwritten with a different
					 * record.  Restart the read by assuming the address to
					 * read is the location where we found this flag; but keep
					 * track of the LSN of the record we were reading, for
					 * later verification.
					 */
					if (pageHeader->xlp_info & XLP_FIRST_IS_OVERWRITE_CONTRECORD)
					{
						state->overwrittenRecPtr = state->DecodeRecPtr;

						/* This is no longer an aborted record. */
						state->abortedRecPtr = InvalidXLogRecPtr;
						state->missingContrecPtr = InvalidXLogRecPtr;

						/* Abandon the record we were trying to assemble. */
						if (state->decoding && state->decoding->oversized)
							pfree(state->decoding);
						state->decoding = NULL;

						/*
						 * Restart the decode at the beginning of the page
						 * where we found the flag; XLREAD_TOT_LEN will skip
						 * over the page header.  PrevRecPtr is left alone, so
						 * the prev-link of the record that overwrote our
						 * contrecord is still validated against the last
						 * record we decoded.
						 */
						state->DecodeRecPtr = targetPagePtr;
						state->record_verified = false;
						state->readRecordState = XLREAD_TOT_LEN;
						goto restart;
					}
